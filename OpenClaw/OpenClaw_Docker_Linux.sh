...
# 稳定版安装 OpenClaw
sudo -i -u $CLAW_USER CURRENT_PROXY="$CURRENT_PROXY" bash << EOF &
# ----------------------
# 设置 npm 用户目录
# ----------------------
mkdir -p ~/.npm-global
...
# 安装 OpenClaw
# ----------------------
npm install -g openclaw@latest --unsafe-perm --registry=https://registry.npmjs.org --loglevel=error > /dev/null 2>&1
# ----------------------
# 初始化 OpenClaw 配置
# ----------------------
...
EOF
REAL_TOKEN=$(cat /home/$CLAW_USER/.openclaw/openclaw.json | grep -oP '"token":\s*"\K[^"]+')
